# План рефакторинга adapter.py и дорожная карта (v2.9.0+)
...
нативные цветные эмодзи. Ветка: `feature/table-render-html-png`.

- **Путь:** генерация HTML из парсера `_parse_table_rows` (`mixins/table_renderer.py::_build_table_html`)
  → скриншот `.wrap` через **Playwright** (Chromium/Chrome) с `device_scale_factor=2` → PNG → upload на MAX.
- **Рендер:** **Playwright — основной** (универсален: и на десктопе, и на Linux-Gateway), **Pillow — единственный
  фоллбэк** при отсутствии браузера. (Использование встроенного браузера Hermes отклонено: `browser_exec` —
  агентский инструмент, недоступен плагину в gateway-процессе.)
...
- **Ограничение ширины:** `table-layout: fixed` + явные `width` на каждую колонку (проценты считает
  ...
  схлопываются. Ширина таблицы ограничена `max-width` (~820px) для мобильных.
