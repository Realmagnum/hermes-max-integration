...
- **SEC-02: токен бота больше не передаётся произвольным URL вложений.** Скачивание входящих audio/image/document переведено на отдельный HTTP-клиент без учётных данных по умолчанию; заголовок `Authorization` добавляется только для явно доверенных HTTPS-origin — инфраструктуры MAX (суффиксы `.max.ru`, `.oneme.ru`, `.okcdn.ru`, `.cdn-max.ru` — те же, что доверяет путь загрузки) и хостов из `MAX_TRUSTED_DOWNLOAD_HOSTS` / `extra["trusted_download_hosts"]` (запись `*.example.com` покрывает только поддомены, не apex). Для остальных origin файл скачивается без токена, редиректы не следуются, а токен вырезается из текста ошибки перед логированием (`_redact_secrets`). Регрессии: `tests/test_download_token_leak.py`.
...
- **Тесты: 167** (154 на момент релиза; актуальное число определяется `pytest --collect-only`): +11 SSRF-guard кейсов, +2 кэш таблиц, +12 нормализация аудио и wire-регрессии
