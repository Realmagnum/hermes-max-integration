...
- **SEC-05: кросс-платформенные сессии выключены по умолчанию и owner-only.** `/sessions` и `/resume` перехватывались до ядра безусловно (`cross_session` по умолчанию `true`) и выдавали title/preview/ID сессий всех платформ любому, кого пропускал общий allowlist — включая случай с пустым `MAX_ALLOWED_USERS` при `allow_all_users=false`. Теперь нужен явный opt-in (`MAX_CROSS_SESSION=true` / `cross_session: true`) **и** вызывающий из списка владельцев (`MAX_CROSS_SESSION_USERS` / `cross_session_users`, по умолчанию — непустой `MAX_ALLOWED_USERS`); `allow_all_users` сам по себе доступа не даёт. Проверка выполняется до любых побочных эффектов (запрос `SessionDB`, отправка, перезапись в `/resume --all`), а также продублирована в самом обработчике. Неавторизованный вызывающий получает обычное поведение ядра (только MAX-сессии).
- `/sessions search` без запроса больше не превращается в `/resume --all search` — второе «search» в команде считается формой поиска и показывает подсказку об использовании.
