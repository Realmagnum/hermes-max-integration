...
Этот плагин прошёл две итерации:

**v1.0 — оригинальная разработка (с нуля)**
Первая версия была написана полностью с нуля под конкретные нужды: интеграция Hermes-агента с MAX, голосовая транскрипция, двухшаговая загрузка файлов, интерактивные кнопки, стриминг ответов.

**v2.0 — переработка на базе зрелого проекта**
Позже был найден проект [vladimiraldushin/hermes-max-platform](https://github.com/vladimiraldushin/hermes-max-platform) — более зрелая кодовая база с хорошей архитектурой плагинов, вебхуками и тестами. Вместо того чтобы продолжать разрабатывать v1.0 изолированно, проект был переработан:

- Архитектура и система подписки (webhook/long polling) взята из upstream
- Весь функционал v1.0 (STT, таблицы-картинки, кнопки, стриминг, загрузка файлов) портирован и расширен
- Добавлены новые возможности: отрисовка таблиц в PNG, улучшенный model picker, standalone-отправитель, групповые политики

**Результат:** гибридный продукт, сочетающий надёжную архитектуру upstream с оригинальными наработками v1.0, которых нет ни в одном из исходных проектов по отдельности.
