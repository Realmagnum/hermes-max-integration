...
- **Зависимость:** `playwright` + наличие Chrome/Chromium на машине Gateway. **Решено (2026-08-20): Playwright — основной рендер** универсально, Pillow — единственный фоллбэк. Если браузера нет — плагин автоматически переключается на прежнюю Pillow-реализацию.
  - **Встроенный браузер Hermes отклонён как опция:** `browser_exec` — это инструмент агентской сессии (доступен ЛЛМ-циклу), а не библиотека, доступная плагину в gateway-процессе. Плагин не может его вызвать из `_render_table_html_png`, поэтому полагаемся только на Playwright (универсален: работает и на десктопе, и на Linux-Gateway) → Pillow.
