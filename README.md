...
| `MAX_QUEUE_MAXSIZE` | ❌ | `1000` | Жёсткий предел очереди входящих апдейтов (backpressure) |
| `MAX_MAX_CONCURRENCY` | ❌ | `8` | Максимум сообщений, обрабатываемых одновременно |
| `MAX_OVERLOAD_POLICY` | ❌ | `drop_oldest` | Поведение при переполнении очереди: `drop_oldest` — вытеснить самое старое, `drop_newest` — отклонить новое |
| `MAX_DEDUP_MAX` | ❌ | `5000` | Жёсткий предел таблицы дедупликации (ID сообщений) |
| `MAX_DEDUP_TTL` | ❌ | `300` | Сколько секунд ID сообщения считается дубликатом |

---

## 🚦 Backpressure и дедупликация

Poller (или webhook) никогда не блокируется: входящие апдейты кладутся в **ограниченную очередь**
(`MAX_QUEUE_MAXSIZE`, по умолчанию 1000), а обработчики запускаются с ограничением параллелизма
(`MAX_MAX_CONCURRENCY`, по умолчанию 8). Когда очередь полна, применяется явная политика перегрузки
(`MAX_OVERLOAD_POLICY`):

| Политика | Поведение | Компромисс |
|----------|-----------|------------|
| `drop_oldest` (по умолчанию) | Вытесняется самый старый апдейт, самый свежий сохраняется | При длительной перегрузке теряются старые сообщения |
| `drop_newest` | Новый апдейт отклоняется, накопленный backlog сохраняется | Обработка отстаёт от потока сообщений |

Дедупликация ограничена жёстко: таблица ID сообщений не превышает `MAX_DEDUP_MAX` записей даже в
непрерывном burst (просроченные записи удаляются по TTL `MAX_DEDUP_TTL`, затем вытесняются самые старые).

Метрики доступны в `/health` (webhook-режим): глубина и пик очереди, число вытесненных/отклонённых
апдейтов, активные обработчики и пик параллелизма, размер таблицы дедупликации:
