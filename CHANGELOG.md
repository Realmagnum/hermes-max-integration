...
- **SEC-04: webhook больше не запускается без секрета (fail-closed).** Ранее при пустом
  `MAX_WEBHOOK_SECRET` сервер поднимался с одним лишь warning — публичный endpoint
  принимал любые события, а отправитель брался из JSON, что позволяло подделать
  разрешённый `user_id`. Теперь `_start_webhook()` отказывается стартовать и пишет
  фатальную ошибку `webhook_secret_required`.
- **Заголовок `X-Max-Bot-Api-Secret` проверяется до тела запроса.** Неаутентифицированный
  вызов получает `403` до чтения/разбора JSON (и до проверки размера тела), а
  `_verify_raw_secret` больше не считает пустой секрет совпадением.
- **Явный dev-opt-in `MAX_WEBHOOK_INSECURE_DEV`:** позволяет webhook без секрета только
  при `MAX_WEBHOOK_HOST` = loopback (`127.0.0.1`/`::1`/`localhost`); любой другой хост
  отвергается с ошибкой `webhook_insecure_dev_non_loopback`.
- Тело webhook ограничено `WEBHOOK_MAX_BODY_BYTES` (1 MB) — лишнее отдаёт `413`.
- Интерактивная настройка предупреждает, что без секрета webhook-режим не стартует.
