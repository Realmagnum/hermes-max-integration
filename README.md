...
| `MAX_GROUP_POLICY` | ❌ | `allowlist` | Политика для групп: `open` \| `closed` \| `allowlist` (см. «Групповые политики») |
...
Групповые сообщения (те, в апдейте которых есть `chat_id`) проверяются по `MAX_GROUP_POLICY`. Личные диалоги под эту политику не попадают: даже `closed` не блокирует прямые сообщения и команды в DM.

| Политика | Поведение |
|----------|-----------|
| `open` | Принимаются все групповые сообщения (явный opt-in) |
| `closed` | Групповые сообщения не принимаются вовсе |
| `allowlist` (по умолчанию) | Сообщение принимается, только если оно подходит **под каждый заданный** список |

**Семантика allowlist — AND, а не OR.** Есть два независимых измерения:

- `MAX_GROUP_ALLOWED_USERS` — пользователи;
- `MAX_GROUP_ALLOWED_CHATS` — группы.

Сообщение проходит, если пользователь есть в списке пользователей (когда список задан) **и** группа есть в списке групп (когда список задан). Пустой список ничего не ограничивает, но и не даёт доступа: если оба списка пусты, все групповые сообщения отклоняются (fail-closed). Неизвестное значение `MAX_GROUP_POLICY` приводится к `closed`, поэтому опечатка не открывает доступ.

```bash
# Только пользователь 123456789, в любой группе
MAX_GROUP_ALLOWED_USERS=123456789

# Только группа -1001234567890, любые участники
MAX_GROUP_ALLOWED_CHATS=-1001234567890

# Пользователь 123456789 и только в группе -1001234567890
MAX_GROUP_ALLOWED_USERS=123456789
MAX_GROUP_ALLOWED_CHATS=-1001234567890

# Любые группы (осознанное решение)
MAX_GROUP_POLICY=open
```

> ⚠️ Ранее пустые списки означали «разрешить всех», а два списка объединялись через OR: пользователь из списка попадал в любую группу. Если вы полагались на это поведение, задайте `MAX_GROUP_POLICY=open`.
